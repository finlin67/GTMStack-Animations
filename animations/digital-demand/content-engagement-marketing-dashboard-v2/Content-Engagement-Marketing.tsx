...
const ChannelDetailModal = ({ channel, onClose }: { channel: ChannelData, onClose: () => void }) => {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.95, y: 10 }}
      animate={{ opacity: 1, scale: 1, y: 0 }}
      exit={{ opacity: 0, scale: 0.95, y: 10 }}
      className="absolute inset-4 z-50 bg-[#151022]/95 backdrop-blur-2xl border border-slate-700/50 rounded-2xl p-6 flex flex-col shadow-2xl"
    >
        {/* Header */}
        <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl ${channel.bgClass} flex items-center justify-center ${channel.textClass} shadow-lg`}>
                    <channel.icon className="w-6 h-6" />
                </div>
                <div>
                    <h3 className="text-xl font-bold text-slate-100 leading-tight">{channel.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs text-slate-400 font-medium">Campaign Performance</span>
            ...
            <button 
                onClick={onClose} 
                className="p-2 -mr-2 -mt-2 hover:bg-white/5 rounded-full transition-colors text-slate-400 hover:text-white"
            >
                <X className="w-5 h-5" />
            </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-[#0F0A19]/50 p-4 rounded-xl border border-slate-800">
                <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 tracking-wider">Impressions</p>
                <p className="text-2xl font-bold text-white tracking-tight">{channel.details.impressions}</p>
            </div>
             <div className="bg-[#0F0A19]/50 p-4 rounded-xl border border-slate-800">
                <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 tracking-wider">Clicks</p>
                <p className="text-2xl font-bold text-white tracking-tight">{channel.details.clicks}</p>
            </div>
             <div className="bg-[#0F0A19]/50 p-4 rounded-xl border border-slate-800">
                <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 tracking-wider">CTR</p>
                <p className="text-2xl font-bold text-emerald-400 tracking-tight">{channel.details.ctr}</p>
            </div>
             <div className="bg-[#0F0A19]/50 p-4 rounded-xl border border-slate-800">
                <p className="text-[10px] uppercase text-slate-500 font-bold mb-1 tracking-wider">Top Region</p>
                <p className="text-lg font-bold text-white tracking-tight truncate">{channel.details.topRegion}</p>
            </div>
        </div>
        
        {/* Visualization Area */}
        <div className="flex-1 bg-[#0F0A19]/30 rounded-xl border border-slate-800/50 p-4 flex flex-col">
            <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Engagement Trend</span>
                <div className="flex gap-1">
                    {[1,2,3].map(i => <div key={i} className="w-1 h-1 rounded-full bg-slate-700"></div>)}
                </div>
            </div>
            <div className="flex-1 flex items-end gap-1.5">
                {[45, 60, 40, 75, 55, 85, 50, 70, 90, 65, 80, 55, 95, 75, 60].map((h, i) => (
                    <motion.div 
                        key={i}
                        initial={{ height: 0 }}
                        animate={{ height: `${h}%` }}
                        transition={{ delay: i * 0.05, duration: 0.4 }}
                        className={`flex-1 rounded-t-sm opacity-80 hover:opacity-100 transition-opacity ${channel.barClass}`} 
                    />
                ))}
            </div>
        </div>
...
const channels: ChannelData[] = [
    {
        ...
        details: { impressions: '84.2K', clicks: '3,204', ctr: '3.8%', topRegion: 'North America' }
    },
    {
        id: 'linkedin',
        name: 'LinkedIn',
        ...
        details: { impressions: '42.1K', clicks: '1,842', ctr: '4.3%', topRegion: 'Europe' }
    }
];
                ...
                <div className="bg-[#1a1122]/60 border border-slate-800/60 rounded-xl p-3 backdrop-blur-md space-y-3">
                    {channels.map(channel => (
                        <button 
                            ...
                            className="w-full flex items-center gap-3 p-1.5 rounded-lg hover:bg-white/5 transition-colors group text-left"
                        >
                            <div className={`w-8 h-8 rounded flex items-center justify-center transition-transform group-hover:scale-110 ${channel.bgClass} ${channel.textClass}`}>
                                <channel.icon className="w-4 h-4" />
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between text-[10px] font-semibold mb-1">
                                    <span className="group-hover:text-white transition-colors">{channel.name}</span>
                                    <span className="text-slate-300">{channel.percentage}%</span>
                                </div>
                                <div className="h-1 bg-slate-800 rounded-full overflow-hidden">
                                    <motion.div 
                                        initial={{ width: 0 }} 
                                        animate={{ width: `${channel.percentage}%` }} 
                                        className={`h-full ${channel.barClass}`} 
                                    />
                                </div>
                            </div>
                            <ArrowRight className="w-3 h-3 text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity -ml-1" />
                        </button>
                    ))}
                </div>
            </div>
