...
/**
 * PRODUCTION-READY STANDALONE LIBRARY: HolographicControlSurface
 * Optimized for: 600x600px container, smooth SVG animations, dual-theme support.
 */
...
function useTheme() {
  ...
  const colors = useMemo(() => ({
    ...
    highlightPanel: isDark
      ...
      : 'bg-gradient-to-br from-indigo-50 to-white border border-indigo-100 shadow-[0_8px_30_rgb(79,70,229,0.08)]'
  }), [isDark]);
...
const Sparkline = React.memo(({ data, colorHex, label, isDark }: { data: number[], colorHex: string, label: string, isDark: boolean }) => {
  ...
  return (
    <div className="relative flex items-center justify-center cursor-crosshair group py-1 h-7" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <svg width={width} height={height} className="overflow-visible">
        ...
        <motion.polyline points={points} fill="none" stroke={colorHex} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1, ease: "easeOut" }} />
      </svg>
      <AnimatePresence>
        {hovered && (
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 5 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 5 }} className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 ${isDark ? 'bg-slate-900/95 border-slate-700' : 'bg-white/95 border-slate-200'} border rounded text-[10px] font-mono z-20 pointer-events-none shadow-xl min-w-[60px] text-center`}>
    ...
    <header className={`flex items-center justify-between px-6 py-4 relative z-50 border-b ${colors.border} ${isDark ? 'bg-[#0F172A]/90' : 'bg-white/90'} backdrop-blur-xl h-[68px] shrink-0`}>
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col">
          ...
          <p className={`text-[10px] font-medium opacity-60 ${colors.muted}`}>Regulatory Node 0x72</p>
        </div>
      </div>
      <div className="flex items-center gap-4">
        <div className="flex flex-col items-end pr-4 border-r border-white/10">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[10px] font-bold ${colors.text}`}>LIVE</span>
          </div>
          <span className={`text-[9px] font-mono opacity-40 ${colors.text}`}>{latency}ms</span>
        </div>
        <button onClick={() => setTheme((t: any) => t === 'light' ? 'dark' : 'light')} className={`p-2 rounded-full transition-all ${isDark ? 'bg-slate-800 text-slate-400 hover:text-white' : 'bg-slate-100 text-slate-500 hover:text-slate-900'}`}>
...
export default function HolographicControlSurface() {
  ...
  return (
    <div className={`w-[600px] h-[600px] max-w-[600px] max-h-[600px] overflow-hidden relative font-['Manrope'] antialiased flex flex-col transition-colors duration-700 ${isDark ? 'bg-[#0F172A] text-slate-300' : 'bg-[#F8FAFC] text-slate-600'}`}>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className={`absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_50%_0%,${isDark ? 'rgba(99,102,241,0.08)' : 'rgba(226,232,240,0.4)'}_0%,transparent_70%)]`} />
        {isDark && (
          <motion.div 
        ...
        <div className="grid grid-cols-12 gap-4 h-[150px]">
          <div className={`col-span-8 ${colors.highlightPanel} rounded-2xl p-6 relative overflow-hidden flex flex-col justify-center group border ${isDark ? 'border-white/10' : 'border-indigo-100'}`}>
            <div className="relative z-10">
              <span className={`inline-block px-2 py-0.5 rounded text-[8px] font-bold uppercase tracking-[0.1em] mb-2 ${isDark ? 'bg-indigo-500/20 text-indigo-300' : 'bg-indigo-50 text-indigo-600'}`}>Sector Active</span>
              ...
              <p className="text-[11px] font-medium opacity-70 mt-1 max-w-[200px]">AI-driven synthesis of multi-regional clinical compliance protocols.</p>
              <button className={`mt-4 px-3 py-1.5 rounded-lg text-[9px] font-bold flex items-center gap-2 transition-all ${isDark ? 'bg-white text-slate-900 hover:bg-slate-200' : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200'}`}>
                SYNTHESIZE <ArrowRight size={10} strokeWidth={3} />
              </button>
            </div>
            <Activity className="absolute right-[-20px] bottom-[-20px] opacity-[0.03] rotate-[-15deg] group-hover:scale-110 transition-transform duration-1000" size={140} />
          </div>
        ...
        <div className={`${colors.panel} rounded-2xl flex-1 flex flex-col overflow-hidden border ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
          <div className={`px-5 py-3 border-b ${colors.border} flex justify-between items-center bg-white/5`}>
            <div className="flex items-center gap-2">
              ...
              <h3 className={`text-[10px] font-bold uppercase tracking-widest ${colors.heading}`}>Metrics Engine</h3>
            </div>
          </div>
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
            <table className="w-full text-left">
              <thead className={`text-[8px] font-bold uppercase tracking-[0.15em] ${colors.muted}`}>
                <tr>
              ...
              <tbody className="text-[10px] font-medium">
                <tr className={`border-b border-white/5 ${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'} transition-colors`}>
                  <td className={`px-3 py-4 ${colors.heading}`}>Clinical Validation</td>
                  <td className="px-3 py-4"><span className="text-emerald-400 font-bold font-mono">92.4%</span></td>
                  <td className="px-3 py-4 w-[130px]"><Sparkline data={historyData.completion} colorHex={colors.primary} label="comp" isDark={isDark} /></td>
                </tr>
                <tr className={`${isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'} transition-colors`}>
                  <td className={`px-3 py-4 ${colors.heading}`}>RWD Aggregation</td>
                  <td className="px-3 py-4"><span className="text-orange-400 font-bold font-mono">48.2%</span></td>
                  <td className="px-3 py-4 w-[130px]"><Sparkline data={historyData.harvesting} colorHex={colors.secondary} label="harv" isDark={isDark} /></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 h-[140px]">
          <div className={`${colors.panel} rounded-2xl p-5 border ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
            ...
            <div className="space-y-2">
              {[
                { name: 'HIPAA Layer', color: colors.success, icon: ShieldCheck },
                { name: 'ISO-27001', color: colors.tertiary, icon: RefreshCw }
              ].map((item) => (
                <div key={item.name} className={`flex items-center justify-between p-2 rounded-lg ${isDark ? 'bg-white/5' : 'bg-slate-50'}`}>
                  <div className="flex items-center gap-2">
          ...
          <div className={`${colors.panel} rounded-2xl p-5 border ${isDark ? 'border-white/10' : 'border-slate-100'}`}>
            <h3 className={`text-[10px] font-bold uppercase tracking-widest ${colors.heading} mb-3`}>Journey</h3>
            <div className="flex items-end justify-between gap-2 h-14">
              {['A', 'B', 'C', 'D'].map((node, idx) => (
                <div key={node} className="flex-1 flex flex-col items-center gap-1">
                  <div className={`w-full rounded-t-sm relative ${isDark ? 'bg-white/5' : 'bg-slate-100'}`} style={{ height: `${[100, 70, 40, 20][idx]}%` }}>
                    <motion.div 
                      initial={{ scaleY: 0 }} 
                      animate={{ scaleY: 1 }} 
                      className="absolute inset-0 origin-bottom rounded-t-sm" 
                      style={{ backgroundColor: idx < 2 ? colors.primary : colors.muted, opacity: 0.6 }} 
                    />
                  </div>
                </div>
              ))}
      ...
      <footer className={`h-[32px] px-6 flex items-center justify-between text-[8px] font-bold uppercase tracking-[0.2em] border-t ${colors.border} ${isDark ? 'bg-slate-900/50 text-slate-500' : 'bg-white text-slate-400'} shrink-0`}>
        <span>System Log: Clear</span>
        <div className="flex gap-4">
          <button className="hover:text-indigo-400 transition-colors">Documentation</button>
          <span className="opacity-30">|</span>
          <span>©2024 Pulse v2.4</span>
        </div>
      </footer>
    </div>
  );
