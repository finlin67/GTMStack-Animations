...
import { ArrowUp, MoreHorizontal, TrendingUp, User, PieChart, Activity, Calendar, ChevronDown, Check, Clock, UserPlus } from 'lucide-react';
...
interface DashboardData {
  ...
  funnel: {
    applied: { val: string; width: string };
    admitted: { val: string; width: string };
    enrolled: { val: string; width: string };
  };
  metrics: {
    ltv: string;
    cac: string;
...
export default function FleetTile() {
  ...
  // Placeholder data for recent enrollments
  ...
  return (
    ...
    >
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col p-6 min-h-0 z-10 relative" onClick={() => setIsDropdownOpen(false)}>
