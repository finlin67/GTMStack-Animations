...
export default function AutoTile() {
  ...
  const [appts, setAppts] = useState(28);
  ...
  useEffect(() => {
    ...
    const liveFeedTimer = setInterval(() => {
      ...
      // Fluctuating Pipeline Metrics
      setLeads(prev => Math.max(100, prev + (Math.floor(Math.random() * 7) - 3)));
      setVisits(prev => Math.max(30, prev + (Math.floor(Math.random() * 5) - 2)));
      setAppts(prev => Math.max(10, prev + (Math.floor(Math.random() * 3) - 1)));
  ...
  return (
    <div className="relative w-full max-w-[600px] aspect-square bg-[#020617] rounded-xl overflow-hidden border border-slate-800 shadow-2xl bg-[radial-gradient(circle,rgba(251,191,36,0.07)_1px,transparent_1px)] bg-[length:24px_24px] group select-none">
      ...
      <motion.div 
        ...
        className="absolute top-8 w-full z-20 text-center pointer-events-none"
      >
        <h3 className="text-amber-500/60 text-xs font-bold uppercase tracking-widest mb-1">Performance Overview</h3>
        <h2 className="text-white text-2xl font-bold drop-shadow-md">Dealership Growth Visualization</h2>
      </motion.div>
      ...
      <div className="absolute inset-0 z-10 p-8 flex flex-col justify-between pointer-events-none">
        ...
        <div className="flex justify-between items-start mt-12">
          {/* Top Left Card: Conversion */}
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 1.4 }}
            className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-xl p-4 w-44 hover:border-amber-500/30 transition-colors duration-300 pointer-events-auto"
          >
        ...
        <div className="flex justify-between items-end mb-4">
          ...
          {/* Bottom Left Card: Pipeline Health */}
          <motion.div 
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 2.0 }}
            className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-xl p-4 w-48 hover:border-amber-500/30 transition-colors duration-300 pointer-events-auto"
          >
            <p className="text-amber-500/70 text-xs font-bold uppercase mb-3">Pipeline Health</p>
            
            {/* Metric 1 */}
            <div className="mb-3">
              <div className="flex justify-between items-center text-xs mb-1">
                <span className="text-slate-300 flex items-center gap-1"><Users className="w-3 h-3 text-orange-400" /> New Leads</span>
                <span className="text-white font-bold">{leads}</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-1.5">
                <div className="bg-orange-500 h-1.5 rounded-full" style={{ width: `${(leads/200)*100}%` }}></div>
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="flex justify-between items-center text-xs mb-1">
                <span className="text-slate-300 flex items-center gap-1"><Store className="w-3 h-3 text-amber-400" /> Visits</span>
                <span className="text-white font-bold">{visits}</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-1.5">
                <div className="bg-amber-500 h-1.5 rounded-full" style={{ width: `${(visits/100)*100}%` }}></div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center text-xs mb-1">
                <span className="text-slate-300 flex items-center gap-1"><Calendar className="w-3 h-3 text-yellow-400" /> Appts</span>
                <span className="text-white font-bold">{appts}</span>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-1.5">
                <div className="bg-yellow-500 h-1.5 rounded-full" style={{ width: `${(appts/50)*100}%` }}></div>
              </div>
            </div>

          </motion.div>

          {/* Bottom Right Card: Retention */}
          <motion.div 
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 1.8 }}
            className="bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-xl p-4 w-44 text-right hover:border-amber-500/30 transition-colors duration-300 pointer-events-auto"
          >
